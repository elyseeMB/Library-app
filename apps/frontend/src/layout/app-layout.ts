...
export class LayoutApp extends LitElement {
  static styles = css`
    ...
    .sidebar {
      ...
      overflow: auto;
      background: var(--bg-sidebar, #f9f8f7);
      border-right: 1px solid var(--separator, #edeff2);
    }
    ...
    .nav a:hover {
      background: var(--gray-50, #edeff2);
    }

    .nav a[aria-current='page'] {
      color: var(--color-neon, #683ffc);
    }
  ...
  private router = new Router(this, [
    ...
    { path: '/livres', render: () => html`<books-page></books-page>` },
    { path: '/auteurs', render: () => html`<authors-page></authors-page>` },
    { path: '/adherents', render: () => html`<members-page></members-page>` },
    { path: '/emprunts', render: () => html`<loans-page></loans-page>` },
  ]);

  private icon(item: NavItem): TemplateResult {
    return html`
      <svg
  ...
  render(): TemplateResult {
    ...
    return html`
      <div class="layout">
        <aside class="sidebar">
          <div class="brand">Bibliothèque</div>
          <nav class="nav">
            ${NAV_ITEMS.map(
              (item) => html`
                <a
                  href=${this.router.link(item.path)}
